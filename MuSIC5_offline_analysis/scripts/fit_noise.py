...
l_bound=6000
u_bound=17000
func_fmt = "[1] + [1] * sin((2*pi/[2])*(x-[3])) * sin((2*pi/[4])*(x-[5]))"

@EntryLogger
# 440000 TDC level number of bins
def init_tree(run_info, bins=5500):
...
def get_initial_settings(hist):
  ...
  res = (("N_{b}",    lambda x: h_max/10.0, lambda x:    1.0, lambda x:   h_max), # Scale factor
         ...
         ("T_{f}",    lambda x:       65.0, lambda x:    0.0, lambda x:  2000.0), # fast period
         ("#phi_{f}", lambda x:        0.0, lambda x:   -0.6, lambda x:     0.6), # fast phase
         ("T_{s}",    lambda x:     2000.0, lambda x: 2000.0, lambda x: 20000.0), # slow period
         ("#phi_{s}", lambda x:        0.0, lambda x:   -0.6, lambda x:     0.6)) # slow phase
  return res       

def fit_hist(hist, fit_options="MER"):
...
def main():
  ...
  for run in run_info:
    ...
    for hist in tree.hists.values():
      ...
      hist.Draw()
      fit_hist(hist)
      print "Chi^2/NDF {}/{}".format(hist.fit_func.GetChiSquare(), hist.fit_func.GetNDF()) 
      can.Update()
      if debug:
        sleep(5)

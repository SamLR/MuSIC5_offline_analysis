...
from fit_settings import fitting_settings_med, fitting_parameters
...
if __name__=="__main__":
    ...
    ch = "D1" 
    sim = "Air_0mm"
    selection = ("file_%i+ch_%s"%(run,ch), "file_%s+ch_na"%sim)
    
    fit_settings =  fitting_settings_med
    ...
    for param in hists_to_draw:
        ...
        dat_hist = hists_to_draw[param][param+"+"+selection[0]]
        # sim_hist = hists_to_draw[param][param+"+file_Air_0mm+ch_na"]
        sim_hist = hists_to_draw[param][param+"+"+selection[1]]
    ...
    can.Update()    
    can.SaveAs("images/sim_vs_data_parameter_WRT_settings.svg")
    sleep(sleep_time)

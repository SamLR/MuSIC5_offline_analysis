...
hist_branch_channel::hist_branch_channel(smart_tfile *const out_file, std::string histname, int channel, branch_ids branch, int n_bins, double x_low, double x_high): hist_maker_algorithm(out_file, histname, n_bins, x_low, x_high), channel_m(channel), branch_m(branch) {

	// if this is an adc branch - check that the channel number is valid (i.e between 1 and 13 - see adc_channel_names)
	if (branch_m == branch_adc) {
		if (channel_m < 1 || channel_m > 13) {
			std::cout << "Invalid channel number for ADC branch" << std::endl;
		}
	}
	else if (branch_m == branch_phadc) {
...
void hist_branch_channel::process(midus_entry const * in_entry) {
	...
	// Fill the histogram
	if (branch_m == branch_adc) { // ADC branch
		hist_maker_algorithm::fill_hist(in_entry->get_value_in_branch(branch_m, channel_m - 2)); // convert channel name to index	
	}
	else if (branch_m == branch_phadc) { // PHADC branch
		hist_maker_algorithm::fill_hist(in_entry->get_value_in_branch(branch_m, channel_m)); // channel name will be the same as the index
	}
	else if (branch_m == branch_T0) { // TDC0 branch
		hist_maker_algorithm::fill_hist(in_entry->get_value_in_branch(branch_m, 0)); 
	}
	else if (branch_m > branch_T0) { // TDCx branch

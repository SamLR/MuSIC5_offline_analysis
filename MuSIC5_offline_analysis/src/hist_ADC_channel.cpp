...
hist_ADC_channel::hist_ADC_channel(smart_tfile *const out_file, std::string histname, int n_bins, double x_low, double x_high): hist_maker_algorithm(out_file, histname, n_bins, x_low, x_high) {
}
...
void hist_ADC_channel::process(midus_entry const * in_entry) {
	...
	// Fill the histogram
	hist_maker_algorithm::fill_hist(in_entry->get_value_in_branch(0, 0));
}

...
hist_QDC_channel::hist_QDC_channel(TFile *const out_file, std::string histname, int n_bins, double x_low, double x_high): hist_maker_algorithm(out_file, histname, n_bins, x_low, x_high) {
}
...
void hist_QDC_channel::process(midus_entry const * in_entry) {
	...
	// Fill the histogram
	hist_maker_algorithm::fill_hist(in_entry->get_QDC_value(1));
}

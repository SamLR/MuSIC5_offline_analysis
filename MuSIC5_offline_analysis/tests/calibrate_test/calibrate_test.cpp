// To test the midus_calibrated_entry class
...
int main() {
	...
	// calibrate values
	int (*calib_fn_ptr)(int const, int const))[3] = {&null_calibration, &adc_calibration, &phadc_calibration, &tdc_calinration };
	
	for (int i = 0; i < 3; i++) {
		mid_file->AddCalibration(calib_fn_ptr[i];
	}

...
{ 
    ...
    tree->Branch("ADC"  ,&(branches[0]),"n_ADCch/I:data[n_ADCch]"); 
    tree->Branch("PHADC",&(branches[0]),"n_PHADCch/I:data[n_PHADCch]"); 
    ...
    for(unsigned int event = 0; event < n_events; ++event) {
        ...
        int n_enteries [4] = {2, 1, 1, 5%(event+1), 13%(event+1)};
